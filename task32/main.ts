...
const newUsers: string[] = ["frank", "ALICE", "george", "BOB", "henry"];

for (const newUser of newUsers) {
  const normalizedNewUser = newUser.toLowerCase(); // Convert to lowercase

  if (currentUsers.some((currentUser) => currentUser.toLowerCase() === normalizedNewUser)) {
